"""
DeployMantisEnv — Pydantic models for the OpenEnv-core RL environment.
...
class DeployMantisState(BaseModel):
    """
    Full ground-truth state of the DeployMantisEnv cloud infrastructure.
...
class DeployMantisObservation(BaseModel):
    """
    What the agent observes after each step.

    This is a *partial* projection of the full DeployMantisState: the agent
...
"""
Top-level action type consumed by `DeployMantisEnvironment.step()`.
...
def make_default_state(episode_id: str = "") -> DeployMantisState:
    """Return a fresh DeployMantisState with 5 idle servers and no uncertainty."""
    servers = [
        ServerRecord(
