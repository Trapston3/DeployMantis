...
def _server_for_prefix(env: DeployMantisEnvironment, prefix: str):
    return next(
        server
...
def test_reset_assigns_dynamic_four_character_server_ids() -> None:
    ...
    server_ids = [server.server_id for server in env.deploymantis_state.servers]
...
def test_reset_changes_ids_between_episodes() -> None:
    ...
    env.reset(task_id=0)
    first_ids = tuple(server.server_id for server in env.deploymantis_state.servers)

    env.reset(task_id=0)
    second_ids = tuple(server.server_id for server in env.deploymantis_state.servers)
...
def test_task_zero_mitigation_uses_dynamic_web_server_id() -> None:
    ...
    assert env.deploymantis_state.servers[0].server_id == web_server.server_id
...
def test_task_one_and_hard_mode_follow_dynamic_ids() -> None:
    ...
    env.deploymantis_state.uncertainty_flag[api_idx] = False
