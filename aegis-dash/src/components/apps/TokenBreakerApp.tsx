    ...
    <div className="flex flex-col h-full gap-5 overflow-y-auto no-scrollbar pb-10">
      ...
      <div className="glass rounded-2xl p-6 shrink-0">
        <div className="flex justify-between items-end mb-4">
          ...
          <div className="text-xl font-mono text-[var(--foreground)] font-bold">
            ${totalSpend.toFixed(4)} <span className="text-[var(--aegis-text-muted)] text-sm font-normal">/ ${budget.toFixed(2)}</span>
          </div>
        </div>

        <div className="w-full h-4 bg-white/[0.04] rounded-full overflow-hidden border border-[var(--aegis-glass-border)]">
          <div
            ...
            style={{
              ...
              background: percentUtilized > 85 
                ...
                : percentUtilized > 50 
                  ...
                  : "linear-gradient(90deg, var(--aegis-accent-primary), #a3b89f)",
            }}
          />
        </div>
      ...
      <div className="flex flex-col lg:flex-row gap-5 shrink-0">
        ...
        <div className="glass-card rounded-xl p-5 lg:w-1/3 flex flex-col justify-between">
          <div>
            ...
            <div className="flex items-center gap-3 mb-6">
              <span className="text-[var(--aegis-text-muted)] font-mono text-lg">$</span>
              <input
                type="number"
                step="0.01"
                min="0.01"
                ...
                className="w-full bg-black/20 border border-[var(--aegis-glass-border)] rounded-lg px-3 py-2 font-mono text-[var(--foreground)] focus:outline-none focus:border-[var(--accent)] transition-colors"
              />
            </div>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 lg:w-2/3">
          <div className="metric-card flex flex-col justify-center">
            ...
            <div className="text-xs text-[var(--aegis-text-muted)] mt-2 uppercase tracking-wider">Total Agents</div>
          </div>
          <div className="metric-card flex flex-col justify-center">
            <div className="text-3xl font-bold font-mono text-[var(--foreground)]">${totalSpend.toFixed(4)}</div>
            <div className="text-xs text-[var(--aegis-text-muted)] mt-2 uppercase tracking-wider">Total Spend</div>
          </div>
          <div className="metric-card flex flex-col justify-center">
            <div className="text-3xl font-bold font-mono" style={{ color: percentUtilized > 85 ? '#ef4444' : percentUtilized > 50 ? '#f59e0b' : 'var(--aegis-accent-primary)' }}>
              ${remainingBudget.toFixed(4)}
            </div>
            <div className="text-xs text-[var(--aegis-text-muted)] mt-2 uppercase tracking-wider">Budget Remaining</div>
          </div>
        </div>
      </div>
      ...
      <div className="glass rounded-xl overflow-hidden shadow-2xl shrink-0 mt-2">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-black/20 text-[10px] uppercase tracking-widest text-[var(--aegis-text-muted)] border-b border-[var(--aegis-glass-border)]">
          ...
          <tbody>
            {agents.length === 0 ? (
              <tr>
                <td colSpan={4} className="px-5 py-8 text-center text-sm text-[var(--aegis-text-muted)]">
              ...
              agents.map(([agentId, spend]) => {
                ...
                return (
                  <tr key={agentId} className="border-b border-[var(--aegis-glass-border)] hover:bg-white/[0.03] transition-colors">
                    ...
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-3">
                        <span className="font-mono text-[11px] text-[var(--aegis-text-muted)] w-12">{agentPct.toFixed(2)}%</span>
                        <div className="flex-1 h-1.5 bg-black/30 rounded-full overflow-hidden max-w-[100px]">
                          <div
                            className="h-full rounded-full transition-all duration-500 ease-out"
                            style={{
                              width: `${Math.min(100, agentPct)}%`,
                              background: isBlocked ? "#ef4444" : agentPct > 50 ? "#f59e0b" : "var(--aegis-accent-primary)"
                            }}
                          />
                        </div>
                      </div>
