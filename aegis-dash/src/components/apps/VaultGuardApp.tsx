    ...
    <div className="flex flex-col h-full gap-5 overflow-y-auto no-scrollbar pb-10">
      ...
      <div className="flex gap-4 shrink-0">
        <div className="metric-card">
          ...
          <div className="text-xs text-[var(--aegis-text-muted)] mt-1 uppercase tracking-wider">Tokens Scrubbed</div>
        </div>
        <div className="metric-card">
          <div className="text-2xl font-bold font-mono text-[var(--foreground)]">
            {isLoading ? "--" : `${activeCount}/${rules?.length}`}
          </div>
          <div className="text-xs text-[var(--aegis-text-muted)] mt-1 uppercase tracking-wider">Active Rules</div>
        </div>
        <div className="metric-card">
          ...
          <div className="text-xs text-[var(--aegis-text-muted)] mt-1 uppercase tracking-wider">Threat Level</div>
        </div>
      </div>
      ...
      <div className="glass rounded-xl overflow-hidden shadow-2xl shrink-0">
        <table className="w-full text-left border-collapse">
          ...
          <tbody>
            ...
            {rules?.map((rule) => (
              <tr key={rule.id} className="border-b border-[var(--aegis-glass-border)] hover:bg-white/[0.03] transition-colors">
                <td className="px-5 py-3 text-sm text-[var(--foreground)]">{rule.name}</td>
                <td className="px-5 py-3 font-mono text-[11px] text-[var(--aegis-text-muted)]">
      ...
      <div className="glass-card rounded-xl p-5 flex flex-col gap-4 min-h-[250px] shrink-0">
        ...
        <div className="flex flex-col md:flex-row gap-4 h-full">
          <div className="flex-1 flex flex-col gap-3">
            <textarea
              className="w-full flex-1 bg-black/20 border border-[var(--aegis-glass-border)] rounded-lg p-3 text-sm font-mono text-[var(--foreground)] placeholder:text-[var(--aegis-text-muted)] focus:outline-none focus:border-[var(--accent)] transition-colors resize-none min-h-[120px]"
          ...
          <div className="flex-1 flex flex-col">
            <div className="text-[10px] uppercase tracking-widest text-[var(--aegis-text-muted)] mb-2">Result Output</div>
