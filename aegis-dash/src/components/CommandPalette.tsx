    ...
    <>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-[20vh] bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-lg glass-card rounded-xl overflow-hidden shadow-2xl border border-[var(--aegis-glass-border)] animate-slide-in">
            <div className="flex items-center px-4 py-3 border-b border-[var(--aegis-glass-border)] bg-black/20">
              ...
              <input
                ...
                className="w-full bg-transparent border-none outline-none text-[var(--foreground)] placeholder:text-[var(--aegis-text-muted)] text-sm font-mono"
              />
            </div>
            <div className="max-h-[60vh] overflow-y-auto p-2">
              {filteredActions.length === 0 ? (
                <div className="px-4 py-3 text-sm text-[var(--aegis-text-muted)] text-center font-mono">
                  No commands found.
                </div>
              ) : (
                filteredActions.map((action, index) => (
                  <button
                    ...
                    className={`w-full flex items-center px-4 py-3 rounded-lg text-sm font-mono transition-colors ${
                      index === selectedIndex
                        ? "bg-[var(--accent)]/20 text-[var(--foreground)] border border-[var(--accent)] shadow-[0_0_10px_var(--aegis-accent-glow)]"
                        : "text-[var(--aegis-text-muted)] hover:bg-white/5 border border-transparent"
                    }`}
                  >
            ...
            <div className="px-4 py-2 border-t border-[var(--aegis-glass-border)] bg-black/20 text-[10px] text-[var(--aegis-text-muted)] flex justify-between font-mono">
