...
const SERVICE_LABELS: Record<string, string> = {
  ...
  "aegis-dash": "AegisDash",
};

const SERVICE_DESCRIPTIONS: Record<string, string> = {
  strata: "Instruments HTTP traffic and exports structured logs & metrics.",
  "aegis-env": "RL Survival Environment validating agent logic.",
  ...
  "aegis-dash": "Enterprise SRE Dashboard.",
};
    ...
    <aside className="w-[260px] min-w-[240px] glass flex flex-col p-4 gap-6 shrink-0 border-r-0 rounded-r-2xl">
      ...
      <div className="pt-1">
        <h1 className="text-xl font-bold tracking-widest text-[var(--foreground)] uppercase">
          Aegis<span className="text-[var(--accent)]">Suite</span>
        </h1>
        <p className="text-[10px] text-[var(--aegis-text-muted)] tracking-[0.2em] mt-0.5">
          COMMAND CENTER
        </p>
      </div>
      ...
      <div className="flex-1 space-y-5 overflow-y-auto no-scrollbar">
        {/* Apps Launcher */}
        <div>
          <h2 className="text-[10px] uppercase tracking-[0.2em] text-[var(--aegis-text-muted)] mb-3 pb-1 border-b border-[var(--aegis-glass-border)]">
            Apps
          </h2>
            ...
            {[
              ...
              return (
                <li key={app.id}>
                  ...
                  >
                    <Icon className={`w-3.5 h-3.5 ${isActive ? "text-[var(--accent)]" : "text-[var(--aegis-text-muted)]"}`} />
        ...
        <div>
          ...
          <ul className="space-y-1.5">
                ...
                <Tooltip.Provider key={svc.name}>
                  <Tooltip.Root delayDuration={300}>
                    <Tooltip.Trigger asChild>
                      ...
                      >
                        <div className="flex items-center gap-2.5">
                          <Icon className="w-3.5 h-3.5 text-[var(--aegis-text-muted)]" />
                          ...
                          {!isSelf && (
                            <button
                              ...
                              className={`
                                ...
                                hover:bg-[var(--accent)]/10 text-[var(--aegis-text-muted)] hover:text-[var(--accent)]
                                disabled:opacity-30
                              `}
                    ...
                    <Tooltip.Portal>
                      ...
                      >
                        {SERVICE_DESCRIPTIONS[svc.name] || "Aegis Microservice."}
            ...
            {services.length === 0 && (
              <li className="text-xs text-[var(--aegis-text-muted)] px-2 py-4 text-center">
        ...
        <div>
          ...
          <div className="glass-card rounded-xl p-3.5 space-y-3">
            {Object.entries(ledger).length > 0 ? (
                ...
                return (
                  <div key={agent} className="space-y-1.5">
                    <div className="flex justify-between items-center">
                      <span className="text-xs text-[var(--aegis-text-muted)] font-mono truncate max-w-[120px]">
                        {agent}
                      </span>
                      <div className="flex items-baseline gap-1.5">
                        ...
                        <span className="text-[9px] text-[var(--aegis-text-muted)]">
                          / ${budget.toFixed(2)}
                        </span>
                      </div>
                    </div>
              ...
              <p className="text-xs text-[var(--aegis-text-muted)] text-center py-2">
              ...
              {nodeStats.map((stat: any) => (
                <div key={stat.name} className="space-y-1.5">
                  <div className="flex justify-between items-center text-[10px]">
                    ...
                    <span className="text-[var(--aegis-text-muted)] font-mono text-[9px]">C:{stat.cpu_percent}% M:{stat.mem_percent}%</span>
                  </div>
                    ...
                    <div className="flex-1 bg-white/[0.04] rounded-l overflow-hidden">
                      <div
                        ...
                        style={{
                          ...
                          background: stat.cpu_percent > 85 ? "#ef4444" : stat.cpu_percent > 50 ? "#f59e0b" : "var(--aegis-accent-primary)"
                        }}
                      />
                    </div>
      ...
      {/* Footer */}
      <div className="text-[9px] text-[var(--aegis-text-muted)] font-mono leading-relaxed opacity-60">
        SYS.STATUS // NOMINAL
        <br />
